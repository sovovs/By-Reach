...
4. 仍失败或视频本来就没有字幕：`by-reach transcribe "URL"` 下载音频转写。
...
by-reach transcribe "https://www.youtube.com/watch?v=VIDEO_ID"
by-reach transcribe ./local_audio.mp3 -o /tmp/transcript.txt
```

> `by-reach transcribe` 只接收公开 http(s) URL 或本地音频文件。用 `ytsearch5:` 搜索时，先从 yt-dlp 结果里选出具体视频 URL，再转写。
> 需要先配置 key：`by-reach configure groq-key`（隐藏输入；免费，console.groq.com）
> 或 `by-reach configure openai-key`。默认 auto 模式只使用第一个已配置服务商
...
# 下载音频并切分为 ASR-ready WAV（无字幕时配合 by-reach transcribe 转写）
bili audio BVxxx
```
...
~/.by-reach/tools/xiaoyuzhou/transcribe.sh --polish "https://www.xiaoyuzhoufm.com/episode/EPISODE_ID"
```
...
3. **配置 Key**: `by-reach configure groq-key`（隐藏输入）
4. **首次运行**: `by-reach install --env=auto --system --channels=xiaoyuzhou`（需用户明确授权）
...
| 场景 | 推荐工具 |
|-----|---------|
| YouTube 字幕 | yt-dlp；失败时 OpenCLI（最多 3 次）→ by-reach transcribe |
| B站视频详情/搜索 | bili-cli |
| B站字幕 | opencli bilibili subtitle |
| 播客转录 | 小宇宙 transcribe.sh |
| 无字幕音视频 | by-reach transcribe（B站音频先 `bili audio`） |
