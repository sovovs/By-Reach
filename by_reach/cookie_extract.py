...
def _require_browser_extractable(spec: PlatformSpec) -> None:
    ...
    if manual_key:
        raise ValueError(
            ...
            f"`by-reach configure {manual_key}`."
        )
...
def _read_xfetch_session(path: Path) -> dict:
    ...
    from by_reach.utils.paths import read_small_text_no_follow
...
def _sync_xfetch_session(auth_token: str, ct0: str) -> bool:
    ...
    try:
        ...
        # Non-fatal: by-reach config is the source of truth, xfetch sync is best-effort
        return False
...
def _sync_bird_env(auth_token: str, ct0: str) -> bool:
    ...
    try:
        ...
        # Non-fatal: by-reach config is the source of truth, bird env sync is best-effort
        return False
