# -*- coding: utf-8 -*-
"""
By-Reach MCP Server — expose doctor/status as MCP tool.

Run: python -m by_reach.integrations.mcp_server

By-Reach is an installer + doctor tool. For actual reading/searching,
...
from by_reach import DISPLAY_NAME, PROGRAM_NAME
...
def create_server():
    if not HAS_MCP:
        print(
            ...
            "'by-reach[mcp] @ "
            "https://github.com/sovovs/By-Reach/archive/main.zip'",
            file=sys.stderr,
        )
        sys.exit(1)

    server = Server(PROGRAM_NAME)
    ...
    async def list_tools():
        return [
            Tool(name="get_status",
                 description=f"Get {DISPLAY_NAME} status: which channels are installed and active.",
