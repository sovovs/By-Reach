...
# Try env var first, then by-reach config.yaml
if [ -z "$GROQ_API_KEY" ]; then
    CONFIG_FILE="$HOME/.by-reach/config.yaml"
    if [ -f "$CONFIG_FILE" ]; then
        ...
        GROQ_API_KEY=$(BY_REACH_CONFIG_FILE="$CONFIG_FOR_PYTHON" \
            "${PYTHON_CMD[@]}" -c 'import os, yaml; print((yaml.safe_load(open(os.environ["BY_REACH_CONFIG_FILE"])) or {}).get("groq_api_key", ""))' \
            2>/dev/null || true)
    fi
fi
GROQ_API_KEY="${GROQ_API_KEY:?请设置 GROQ_API_KEY 环境变量或运行 by-reach configure groq-key}"
...
if ! WORK_DIR=$(mktemp -d "${TEMP_ROOT%/}/by-reach-xiaoyuzhou.XXXXXX"); then
    echo "❌ 无法创建临时目录" >&2
    exit 1
fi
...
for i in $(seq 0 $((NUM_CHUNKS - 1))); do
    ...
    if [ "$HTTP_CODE" != "200" ]; then
        ...
        if [ "$HTTP_CODE" = "429" ]; then
            ...
            case "$HTTP_CODE" in
                5??)
                    echo "   可尝试兜底：by-reach transcribe \"$AUDIO_URL\""
                    ;;
            esac
            exit 1
        fi
...
def call_groq(text):
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "by-reach-xiaoyuzhou/1.0",
        },
    )
...
if [ -z "$OUTPUT" ]; then
    if ! OUTPUT=$(mktemp "${TEMP_ROOT%/}/by-reach-transcript.XXXXXX"); then
        echo "❌ 无法安全创建输出文件" >&2
        exit 1
    fi
fi
