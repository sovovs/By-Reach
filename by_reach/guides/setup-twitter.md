...
`~/.by-reach/config.yaml`，供 `by-reach doctor` 检查显式凭据是否齐全。
`doctor` 不会执行 `twitter status`，不会实时验证账号是否可用，也不会修改当前 Shell。

默认只写 `~/.by-reach/config.yaml`。只有用户明确同意复制凭据并显式增加
...
```bash
by-reach configure twitter-cookies --sync-legacy-twitter
```

`by-reach uninstall` 只会提醒这些 legacy 副本，不会自动删除。需要清理时，
先让用户确认，再手工删除上述两个文件。

`twitter` 是独立的上游命令，不会读取 By-Reach 的配置文件。直接运行
...
如果你已经安装了 [bird CLI](https://www.npmjs.com/package/@steipete/bird)（`npm install -g @steipete/bird`），它也能正常工作。By-Reach 会自动检测并使用已安装的 bird。两者功能类似，twitter-cli 是当前推荐方案。
