# -*- coding: utf-8 -*-
"""
By-Reach CLI — installer, doctor, and configuration tool.
...
def main():
    ...
    parser.add_argument("--version", action="version", version=f"{DISPLAY_NAME} v{__version__}")
    ...
    # ── uninstall ──
    p_uninstall = sub.add_parser("uninstall", help="Remove all By-Reach config, tokens, and skill files")
    ...
    p_uninstall.add_argument("--keep-config", action="store_true",
                             help="Remove skill files only, keep ~/.by-reach/ config and tokens")
    ...
    if args.command == "configure" and args.from_browser:
        ...
        if args.platform in manual_keys:
            p_conf.error(
                ...
                f"`by-reach configure {manual_keys[args.platform]} ...`"
            )
    ...
    if args.command == "version":
        print(f"{DISPLAY_NAME} v{__version__}")
        sys.exit(0)
...
def _cmd_install(args):
    ...
    if unknown_channels:
        ...
        print(
            f"by-reach install: error: unknown channel(s): {unknown}. "
    ...
    if env == "server":
        ...
        print("   保存代理供 Agent 使用：by-reach configure proxy（隐藏输入）")
    ...
    if not dry_run:
        ...
        if safe_mode:
            ...
            if not requested_channels:
                ...
                print("   by-reach install --system --channels=twitter,xiaohongshu,reddit,facebook,instagram,...")
                print("   by-reach install --system --channels=all  (install everything)")

            # Star reminder
            print()
            print(f"如果 {DISPLAY_NAME} 帮到了你，给个 Star 让更多人发现它吧：")
            print("   https://github.com/sovovs/By-Reach")
...
def _install_skill(force: bool = True):
    """Install By-Reach as an agent skill for supported agent clients."""
    ...
    def _skill_resource_name() -> str:
        locale_candidates = (
            os.environ.get(f"{ENV_PREFIX}LANG", ""),
    ...
    def _copy_skill_dir(target: str) -> str | None:
        ...
        try:
            ...
            try:
                skill_pkg = importlib.resources.files("by_reach").joinpath("skill")
    ...
    for skill_dir, platform_name in skill_dirs:
        if os.path.isdir(skill_dir):
            target = os.path.join(skill_dir, SKILL_NAME)
    ...
    if not installed:
        ...
        target = os.path.expanduser(os.path.join("~/.agents/skills", SKILL_NAME))
...
def _uninstall_skill():
    ...
    skill_dirs = [
        (os.path.join("~/.config/opencode/skills", SKILL_NAME), "OpenCode"),
        (os.path.join("~/.openclaw/skills", SKILL_NAME), "OpenClaw"),
        (os.path.join("~/.claude/skills", SKILL_NAME), "Claude Code"),
        (os.path.join("~/.agents/skills", SKILL_NAME), "Agent"),
    ]
    ...
    if openclaw_home:
        skill_dirs.insert(
            0,
            (os.path.join(openclaw_home, ".openclaw", "skills", SKILL_NAME), "OpenClaw"),
        )
...
def _install_system_deps():
    ...
    if shutil.which("deno"):
        ...
        from by_reach.channels.youtube import (
...
def _install_xiaoyuzhou_deps():
    ...
    from by_reach.config import Config
    from by_reach.utils.paths import PrivatePathError, atomic_write_private_text

    config = Config()
    print("Setting up Xiaoyuzhou podcast transcription...")

    tools_dir = os.path.expanduser(f"~/{CONFIG_DIRNAME}/tools/xiaoyuzhou")
    ...
    if has_key:
        ...
        print("     Then run: by-reach configure groq-key（隐藏输入）")
...
def _cmd_configure(args):
    ...
    if args.from_browser:
        from by_reach.cookie_extract import configure_from_browser
        ...
        print()
        if found_any:
            print("✅ Cookies configured! Run `by-reach doctor` to see updated status.")
        else:
    ...
    # ── Manual configure ──
    if not args.key:
        print("Usage: by-reach configure <key> [--stdin]")
        print("   Omit the value to enter it through a hidden prompt.")
        print(
            "   or: by-reach configure --from-browser chrome "
            "--platform xueqiu"
        )
        return
    ...
    if args.key == "proxy":
        ...
        if auth_token and ct0:
            ...
            print("✅ Twitter cookies 已保存到 ~/.by-reach/config.yaml")
            if getattr(args, "sync_legacy_twitter", False):
                from by_reach.cookie_extract import (
                    _sync_bird_env,
...
def _cmd_transcribe(args):
    ...
    from by_reach.transcribe import TranscribeError, transcribe
    from by_reach.utils.text import scrub_url_credentials
...
def _cmd_uninstall(args):
    """Remove all By-Reach config, tokens, and skill files."""
    import shutil
    import subprocess

    from by_reach.utils.paths import home_dir

    dry_run = args.dry_run
    keep_config = args.keep_config

    print()
    print("By-Reach Uninstaller")
    print("=" * 40)
    ...
    # ── 1. Config directory (~/.by-reach/) ──
    config_dir = home_dir() / CONFIG_DIRNAME
    if not keep_config:
    ...
    if dry_run:
        ...
        if removed_any:
            print("By-Reach data removed.")
        elif mcporter_cleanup_skipped:
            print("No proven By-Reach-managed mcporter data was removed.")
        else:
            print("Nothing to remove — already clean.")

    print()
    print("Optional: remove the By-Reach Python package itself:")
    print("  pip uninstall by-reach")
    print()
    print("Optional: remove tools installed by By-Reach:")
...
def _cmd_setup():
    ...
    print("运行 by-reach doctor 查看完整状态")
    print()
...
_UPDATE_INSTRUCTIONS = (
    ...
    "  帮我更新 By-Reach：https://raw.githubusercontent.com/sovovs/By-Reach/main/docs/update.md\n"
    "仅更新本体（不含上游工具和 skill）：\n"
    "  pip install --upgrade https://github.com/sovovs/By-Reach/archive/main.zip"
)


def _is_newer_version(remote: str, local: str) -> bool:
    """True if remote is strictly newer under PEP 440 ordering.
    ...
    window) — and walk them into a downgrade. Invalid versions fail closed.
    """
    try:
...
def _cmd_check_update():
    ...
    from by_reach import __version__

    print(f"当前版本: v{__version__}")
    release_url = "https://api.github.com/repos/sovovs/By-Reach/releases/latest"
    commit_url = "https://api.github.com/repos/sovovs/By-Reach/commits/main"
...
def _cmd_watch():
    ...
    resp, err, _attempts = _github_get_with_retry(
        "https://api.github.com/repos/sovovs/By-Reach/releases/latest",
        timeout=10,
        retries=2,
    )
    ...
    if not issues and not update_available:
        print(f"By-Reach: 全部正常 ({ok}/{total} 渠道可用，v{__version__} 已是最新)")
        return
    ...
    if update_available:
        ...
        print("    帮我更新 By-Reach：https://raw.githubusercontent.com/sovovs/By-Reach/main/docs/update.md")
