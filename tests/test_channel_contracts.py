...
def test_ordered_backends_override_moves_backend_to_front():
    ...
    ch = TwitterChannel()
    ordered = ch.ordered_backends({"twitter_backend": "bycli"})
...
def test_public_backend_override_rejects_prefixes_and_typos(override):
    from by_reach.channels.twitter import TwitterChannel

    channel = TwitterChannel()

    assert channel.ordered_backends({"twitter_backend": override}) == [
        "twitter-cli",
        "bycli",
    ]
